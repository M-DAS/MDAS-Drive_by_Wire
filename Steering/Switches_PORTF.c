...
void PORTD_Setup(void)
{
	...
	***********************/
	...
	//GPIOIntTypeSet(GPIO_PORTD_BASE, GPIO_PIN_6 | GPIO_PIN_7, GPIO_BOTH_EDGES); //PREV: GPIO_FALLING_EDGE
...
void GPIOPortD_Handler(void)
{
	...
	uint32_t status;
	uint32_t buttonPressed = GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_7) & GPIO_PIN_7;

	status = GPIOIntStatus(GPIO_PORTD_BASE, true);
	
	if ((status & GPIO_PIN_7) && (buttonPressed == GPIO_PIN_7))
	{
		g_start_flag = true;
	}
	else if((status & GPIO_PIN_7) && (buttonPressed != GPIO_PIN_7)) //NOT HERE PREVIOUSLY
	{
		g_start_flag = false;
	}
